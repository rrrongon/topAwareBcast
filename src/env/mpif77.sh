...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        $Show $F77 $PROFILE_INCPATHS ${final_fflags} ${final_ldflags} $allargs -I$includedir $ITAC_OPTIONS -L$libdir $f77libs $PROFILE_PRELIB $PROFILE_FOO $rpath_flags -lmpi  $PROFILE_POSTLIB ${final_libs} 
        rc=$?
    fi
else
